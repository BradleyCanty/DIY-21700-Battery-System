...
This battery system is designed such that the battery itself is top-mounted onto the vehicle its powering (and top-mounted onto its charging station). This mounting paradigm was chosen to simplify battery swapping (either by human hand or by robot gripper). As such, the algorithm to swap the battery is as follows:
